...
public class UserController : AuthorizedController // Here we use the AuthorizedController as the base class because it derives ControllerBase and also has useful methods to retrieve user information.
{
    ...
    //    return currentUser.Result != null ?
    //        this.FromServiceResponse(await UserService.GetCartId(currentUser.Result)) :
    //        this.ErrorMessageResult<Guid>(currentUser.Error);
    //}

    [Authorize] // You need to use this attribute to protect the route access, it will return a Forbidden status code if the JWT is not present or invalid, and also it will decode the JWT token.
    [HttpGet("{id:guid}")] // This attribute will make the controller respond to a HTTP GET request on the route /api/User/GetById/<some_guid>.
    public async Task<ActionResult<RequestResponse<ICollection<Guid>>>> GetOrdersIdsById([FromRoute] Guid id) // The FromRoute attribute will bind the id from the route to this parameter.
    {
        var currentUser = await GetCurrentUser();

        return currentUser.Result != null ?
            this.FromServiceResponse(await UserService.GetOrdersIds(currentUser.Result, id)) :
            this.ErrorMessageResult<ICollection<Guid>>(currentUser.Error);
    }

    [Authorize] // You need to use this attribute to protect the route access, it will return a Forbidden status code if the JWT is not present or invalid, and also it will decode the JWT token.
    [HttpGet("{id:guid}")] // This attribute will make the controller respond to a HTTP GET request on the route /api/User/GetById/<some_guid>.
    public async Task<ActionResult<RequestResponse<Guid>>> GetCartById([FromRoute] Guid id) // The FromRoute attribute will bind the id from the route to this parameter.
    {
        var currentUser = await GetCurrentUser();
    ...
    public async Task<ActionResult<RequestResponse<BriefUserDTO>>> Add([FromBody] UserAddDTO user)
    {
        var currentUser = await GetCurrentUser();
        user.Password = PasswordUtils.HashPassword(user.Password);

        return currentUser.Result != null ?
            this.FromServiceResponse<BriefUserDTO>(await UserService.AddUser(user, currentUser.Result)) :
            this.ErrorMessageResult<BriefUserDTO>(currentUser.Error);
    }


    /// <summary>
    ///     This route is meant to add vendor users. It will return brief info on the added user or error in the following
    ///     cases:
    ///         - Only admin users can add enterprise users
    ///         - This route is strictly used for adding Vendor users
    ///         - Cannot add multiple users with the same email address
    ///         - The specified vendor does not exist in the database
    ///         - The specified vendor's contract has expired
    /// </summary>
    /// <param name="user"></param>
    /// <returns></returns>
    [Authorize]
    [HttpPost] // This attribute will make the controller respond to a HTTP POST request on the route /api/User/Add.
    public async Task<ActionResult<RequestResponse<BriefUserDTO>>> AddEnterpriseUser([FromBody] VendorUserAddDTO user)
    {
        var currentUser = await GetCurrentUser();
        user.Password = PasswordUtils.HashPassword(user.Password);

        return currentUser.Result != null ?
            this.FromServiceResponse<BriefUserDTO>(await UserService.AddVendorUser(user, currentUser.Result)) :
            this.ErrorMessageResult<BriefUserDTO>(currentUser.Error);
    }
