import{RegisterFormController, RegisterFormModel} from "./RegisterVendorUserForm.types"
...
export const useRegisterVendorUserFormController = () : RegisterFormController => {
    const { formatMessage } = useIntl();
    const { defaultValues, resolver } = useInitRegisterForm();
    const { registerVendorMutation: { mutation: registerVendorMutation, key: registervendorUserMutationKey } } = useRegisterApi();
    const { redirectToLogin } = useAppRouter();
    const { mutateAsync: add, status } = useMutation({
        mutationKey: [registervendorUserMutationKey], 
        mutationFn: registerVendorMutation
    })
    ...
    const submit = useCallback(async (data: RegisterFormModel) => {
        try {
            ...
            if (error instanceof Error) {
                toast.error(`${formatMessage({ id: "notifications.messages.registerError" })}: ${error.message}`);
            } else {
                toast.error(formatMessage({ id: "notifications.messages.registerError" }));
            }
        }
    }, [add, formatMessage, redirectToLogin]);
